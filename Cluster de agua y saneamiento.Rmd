...
```{r}
set.seed(777)
row.names(Aguaysaneamiento)=Aguaysaneamiento$Pais
Aguaysaneamiento=na.omit(Aguaysaneamiento)
distancia= daisy(Aguaysaneamiento[,2:3],metric = "gower")
```
# Cluster no jerárquico - partición
```{r}
fviz_nbclust(Aguaysaneamiento[,2:3], pam, diss = distancia,method = "gap_stat",k.max = 10,verbose = F)
```

```{r}
pam.resultado = pam(distancia,1,cluster.only = F)
Aguaysaneamiento$clusterAS=pam.resultado$cluster
fviz_cluster(object = list(data=distancia, cluster = Aguaysaneamiento$clusterAS),
             geom = c("text"), 
             ellipse.type = "convex")
```
# Clusters jerarquicos
```{r}
fviz_nbclust(Aguaysaneamiento[,2:3], hcut, diss = distancia,method = "gap_stat",k.max = 10,verbose = F)
```
# Aglomerativo - Agnes
```{r}
res.agnes<-hcut(distancia, k = 1,hc_func='agnes',hc_method = "ward.D")
Aguaysaneamiento$clusterAgnes=res.agnes$cluster
fviz_dend(res.agnes,k=1, cex = 0.5, horiz = T)
```
# Divisivo - Diana
```{r}
res.diana <- hcut(distancia, k = 1,hc_func='diana')
Aguaysaneamiento$clusterDivisivo=res.diana$cluster
...
```
```{r}
Aguaysaneamiento$dim1 <- proyeccionAS$points[,1]
Aguaysaneamiento$dim2 <- proyeccionAS$points[,2]

#limites
min(Aguaysaneamiento[,c('dim1','dim2')]); max(Aguaysaneamiento[,c('dim1','dim2')])
```
```{r}
limites=c(-0.2,2)
g.dist.cmd_AS = daisy(Aguaysaneamiento[,c('dim1','dim2')], metric = 'euclidean')
```
```{r}
library(dbscan)
...
```{r}
library(fpc)
db.cmd_AS= dbscan(g.dist.cmd_AS, eps=0.09, MinPts=2,method = 'dist')
db.cmd_AS
```

```{r}
Aguaysaneamiento$dbCMD_AS=as.factor(db.cmd_AS$cluster)
```


```{r}
library(ggrepel)
base_AS= ggplot(Aguaysaneamiento,aes(x=dim1, y=dim2)) + ylim(limites) + xlim(limites) + coord_fixed()
dbplot_AS= base_AS + geom_point(aes(color=dbCMD_AS))

dbplot_AS
```
```{r}
dbplot_AS + geom_text_repel(size=3,aes(label=row.names(Aguaysaneamiento[2:3])))
```


```{r}
LABEL=ifelse(Aguaysaneamiento$dbCMD_AS==0,row.names(Aguaysaneamiento[2:3]),"")
