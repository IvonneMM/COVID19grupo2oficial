...
```{r}
tanova=anova(reg1cov,reg2cov,reg3cov,reg4cov,reg5cov)
...
Modelo12=formula(muertes~corrupcion+estabilidad+accountability+efectividad+ruleoflaw+regulacion+agua+saneamiento+chepercapita+medicos+UHC+camas+CHE)
```
...
```{r}
tanova_covid=anova(reg1cov,reg2cov,reg3cov,reg4cov,reg5cov,reg6cov,reg7cov,reg8cov,reg9cov,reg10cov,reg11cov,reg12cov)
stargazer(tanova_covid,type = 'text',summary = F,title = 'Tabla ANOVA')
```
```{r}
Modelo13=formula(muertes~agua+saneamiento+chepercapita+CHE+medicos+UHC+camas)
reg13cov=lm(Modelo13,data=Datacovidof)
stargazer(reg13cov,type='text',intercept.bottom = F)
```

```{r}
Modelo14=formula(muertes~estabilidad+accountability+UHC+CHE+medicos)
reg14cov=lm(Modelo14,data=Datacovidof)
stargazer(reg14cov,type='text',intercept.bottom = F)
```

```{r}
Modelo15=formula(muertes~estabilidad+accountability+UHC+CHE)
reg15cov=lm(Modelo15,data=Datacovidof)
stargazer(reg15cov,type='text',intercept.bottom = F)
```
#linealidad
```{r}
plot(reg15cov,1)
...
```{r}
shapiro.test(reg15cov$residuals)
```
#multicolinealidad
```{r}
library(DescTools)
...
checkReg15=as.data.frame(influence.measures(reg15cov)$is.inf)
head(checkReg15)

checkReg15[checkReg15$cook.d | checkReg15$hat,]
```
