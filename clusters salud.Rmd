...
```{r}
set.seed(777)
row.names(DataSalud)=DataSalud$Pais
DataSalud=na.omit(DataSalud)
g.distsalud = daisy(DataSalud[,c(2:6)], metric='gower')
fviz_nbclust(DataSalud[,c(2:6)], pam,diss=g.distsalud,method = 'gap_stat',k.max = 10,verbose = F)
```

```{r}
res.pamSALUD=pam(g.distsalud,1,cluster.only = F)
DataSalud$clusterPAM=res.pamSALUD$cluster
fviz_cluster(object = list(data=g.distsalud, cluster = DataSalud$clusterPAM),
             geom = c('text'), 
             ellipse.type = 'convex')
...
# Jerarquización 
```{r}
fviz_nbclust(DataSalud[,c(2:6)], hcut,diss=g.distsalud,method = 'gap_stat',k.max = 10,verbose = F)
```
# Aglomerativo - Agnes
```{r}
res.agSALUD = hcut(g.distsalud, k = 1,hc_func='agnes',hc_method = 'ward.D')
DataSalud$clusterAG=res.agSALUD$cluster
fviz_dend(res.agSALUD,k=1, cex = 0.7, horiz = T)
```
# Divisivo - Diana
```{r}
res.diaSALUD = hcut(g.distsalud, k = 1,hc_func='diana')
DataSalud$clusterDIV=res.diaSALUD$cluster
fviz_dend(res.diaSALUD, cex = 0.7,horiz = T)
```
# Estrategia basada en densiadad

```{r}
proyeccionSALUD = cmdscale(g.distsalud, k=2,add = T)
DataSalud$dim1 = proyeccionSALUD$points[,1]
DataSalud$dim2 = proyeccionSALUD$points[,2]
min(DataSalud[,c('dim1','dim2')]); max(DataSalud[,c('dim1','dim2')])
```
```{r}
limites=c(-0.4,0.5)
g.dist.cmdSALUD = daisy(DataSalud[,c('dim1','dim2')], metric = 'euclidean')
kNNdistplot(g.dist.cmdSALUD, k=5)
abline(h=0.16, lty=2)
```
```{r}
db.cmdSALUD = dbscan(g.dist.cmdSALUD, eps=0.16, MinPts=5,method = 'dist')
db.cmdSALUD 
```
