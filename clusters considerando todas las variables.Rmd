...
# Clusters considerando todas las variables (Gonzalo Berger)
...
g.distcovid = daisy(Datacovidof[,c(4:17)], metric='gower')
fviz_nbclust(Datacovidof[,c(4:17)], pam,diss=g.distcovid,method = 'gap_stat',k.max = 10,verbose = F)
```
```{r}
res.pamCOVID=pam(g.distcovid,2,cluster.only = F)
Datacovidof$clusterPAM=res.pamCOVID$cluster
fviz_cluster(object = list(data=g.distcovid, cluster = Datacovidof$clusterPAM),
             geom = c('text'), 
             ellipse.type = 'convex')
...
# Jerarquización 
```{r}
fviz_nbclust(Datacovidof[,c(4:17)], hcut,diss=g.distcovid,method = 'gap_stat',k.max = 10,verbose = F)
```

# Aglomerativo - Agnes
```{r}
res.agCOVID = hcut(g.distcovid, k = 2,hc_func='agnes',hc_method = 'ward.D')
Datacovidof$clusterAG=res.agCOVID$cluster
fviz_dend(res.agCOVID,k=2, cex = 0.7, horiz = T) 
```

# Divisivo - Diana
```{r}
res.diaCOVID = hcut(g.distcovid, k = 2,hc_func='diana')
Datacovidof$clusterDIV=res.diaCOVID$cluster
fviz_dend(res.diaCOVID, cex = 0.7,horiz = T)
```

# Evaluación gráfica con siluetas
```{r}
fviz_silhouette(res.pamCOVID)
```

```{r}
fviz_silhouette(res.agCOVID)
```

```{r}
fviz_silhouette(res.diaCOVID)
```

# Evaluación numérica
```{r}
poorPAMof=data.frame(res.pamCOVID$silinfo$widths)
poorPAMof$Pais=row.names(poorPAMof)
poorPAMofcases=poorPAMof[poorPAMof$sil_width<0,'Pais']
poorPAMofcases
length(poorPAMofcases)
```
```{r}
poorAGNESof=data.frame(res.agCOVID$silinfo$widths)
poorAGNESof$Pais=row.names(poorAGNESof)
poorAGNESofcases=poorAGNESof[poorAGNESof$sil_width<0,'Pais']
poorAGNESofcases
length(poorAGNESofcases)
```
```{r}
poorDIANAof=data.frame(res.diaCOVID$silinfo$widths)
poorDIANAof$Pais=row.names(poorDIANAof)
poorDIANAofcases=poorDIANAof[poorDIANAof$sil_width<0,'Pais']
poorDIANAofcases
length(poorDIANAofcases)
```
# Estrategia basada en densiadad
```{r}
row.names(Datacovidof)=Datacovidof$Pais
Datacovidof=na.omit(Datacovidof)
distcovid= daisy(Datacovidof[,4:17],metric = 'gower')
proyeccioncovid=cmdscale(distcovid,k=14,add = T)
Datacovidof$dim1=proyeccioncovid$points[,1]
Datacovidof$dim2=proyeccioncovid$points[,2]
min(Datacovidof[,c('dim1','dim2')]);max(Datacovidof[,c('dim1','dim2')])
```
```{r}
limitescovid=c(-0.6,0.4)
g.dist.cmdcovid=daisy(Datacovidof[,c('dim1','dim2')],metric = 'euclidean')
kNNdistplot(g.dist.cmdcovid, k=14)
abline(h=0.2,lty=2)
```
```{r}
db.cmdcovid = dbscan(g.dist.cmdcovid, eps=0.20, MinPts=14,method = 'dist')
db.cmdcovid 
```
