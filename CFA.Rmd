...
```{r}
dontselect=c("Pais","continentalregion","subregion","Decesos por millón de habitantes")
seleccionados=setdiff(names(Datacovidof),dontselect)
COVIDEFA=Datacovidof[,seleccionados]
```
```{r}
library(polycor)
...
```{r}
library(GPArotation)
resulefa=fa(COVIDEFA,nfactors = 3,cor = 'mixed',rotate = "varimax",fm="minres")
```
```{r}
...
```
# Análisis Factorial Confirmativo (Mayra Vargas)
```{r}
names(COVIDEFA)=gsub(" ","",names(COVIDEFA))
names(COVIDEFA)=gsub("_","",names(COVIDEFA))
names(COVIDEFA)=gsub("%of","",names(COVIDEFA))
names(COVIDEFA)=gsub("/","",names(COVIDEFA))
```

# Modelo
```{r}
modelo <- ' Gobernanza  =~ Controldelacorrupción + Estabilidadpolíticayausenciadeviolenciaterrorismo + VoiceandAccountability + Efectividadgobierno + RuleofLaw + RegulatoryQuality 
           Recursos_materiales_y_humanos =~ populationwithaccesstoimproveddrinkingwater + PorcentajeSaneamiento + numerodecamaspor1000hab + Numeromedicos
           Recursos_financieros   =~ CHE + Porcentajechepercapita + Asistenciasanitariauniversal'
```
```{r}
COVIDEFA=scale(COVIDEFA)
```
```{r}
library(lavaan)
cfa_cov <-cfa(modelo, data = COVIDEFA,
              std.lv=TRUE,
              missing="fiml")
```
#Test
```{r}
allParamCFA_COVID=parameterEstimates(cfa_cov,standardized = T)
allFitCFA_COVID=as.list(fitMeasures(cfa_cov))
```
```{r}
library(knitr)
kable(allParamCFA_COVID[allParamCFA_COVID$op=="=~",])
```
#ChiSquare
```{r}
allFitCFA_COVID[c("chisq", "df", "pvalue")]
```
#Tucker Lewis
```{r}
allFitCFA_COVID$tli
...
```{r}
allFitCFA_COVID[c('rmsea.ci.lower','rmsea' ,'rmsea.ci.upper')]
```
```{r}
COVIDEFA=as.data.frame(cbind(COVIDEFA,lavPredict(cfa_cov)))
summary(COVIDEFA)
```
```{r}
summary(lm(Gobernanza~Recursos_materiales_y_humanos+Recursos_financieros,data = COVIDEFA))
```
```{r}
old=apply(COVIDEFA[,c("Controldelacorrupción","Estabilidadpolíticayausenciadeviolenciaterrorismo","VoiceandAccountability","Efectividadgobierno","RuleofLaw","RegulatoryQuality" )],1,mean)
new=COVIDEFA$gobernanza
plot(old,new)
```

#No hay correlacion entre los factores.
