...
```
```{r}
names(covid19)=c('Pais','Z1')
```
```{r}
covid19$Z1=gsub(',','',covid19$Z1)
```
```{r}
covid19$Z1=gsub('No data',NA,covid19$Z1)
```
```{r}
covid19[,c(2)]=as.numeric(covid19[,c(2)])
```
```{r}
table(covid19$Z1, useNA = 'always')
```

```{r}
names(poblacion)=c('Pais','continentalregion','subregion','Z2')
```
```{r}
poblacion$Z2=gsub(',',"",poblacion$Z2)
```
```{r}
poblacion$Z2=as.numeric(poblacion$Z2)
```
```{r}
table(poblacion$Z2, useNA = 'always')
```
```{r}
